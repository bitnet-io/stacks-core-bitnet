                ...
                for prepare_block_ops in prepare_phase_ops.iter() {
                    ...
                    for opdata in prepare_block_ops.iter() {
                            ...
                            ancestors.get(&(opdata.block_height, opdata.vtxindex))
                        {
                            ...
                            {
                                info!("Block-commit {},{} descends from likely PoX anchor block {},{}", opdata.block_height, opdata.vtxindex, op_ancestor_height, op_ancestor_vtxindex);
                                ...
                                info!("Block-commit {},{} does NOT descend from likely PoX anchor block {},{}", opdata.block_height, opdata.vtxindex, ancestor_block, ancestor_vtxindex);
...
pub fn update_pox_affirmation_maps<B: BurnchainHeaderReader>(
    ...
    info!("Process PoX affirmations for reward cycle {}", reward_cycle);
    ...
    if let Some((anchor_block, descendancy)) = pox_anchor_block_info_opt.clone() {
        ...
        info!("PoX anchor block selected in reward cycle {} is None. Reward cycle {} has no anchor block", reward_cycle, reward_cycle + 1);
