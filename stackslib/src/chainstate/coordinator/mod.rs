            ...
            match self.handle_new_burnchain_block() {
                Ok(burn_block_status) => match burn_block_status {
                    ...
                    NewBurnchainBlockStatus::WaitForPox2x(block_hash) => {
                        info!("Missing canonical Stacks 2.x anchor block {}", &block_hash,);
                    }
                    NewBurnchainBlockStatus::WaitForPoxNakamoto => {
                        info!("Missing canonical Nakamoto anchor block");
                    }
                },
                Err(e) => {
    ...
    > ChainsCoordinator<'a, T, N, U, CE, FE, B>
{
            ...
            while search_weight >= 0 {
                ...
                for hdr in all_headers {
                    ...
                    match StacksChainState::is_block_compatible_with_affirmation_map(
                        ...
                        Ok(compat) => {
                            if !compat {
                                info!("Stacks tip {}/{} affirmation map {} is incompatible with heaviest affirmation map {}",
                    ...
                    let ancestor_sn = match SortitionDB::get_ancestor_snapshot_tx(
                        ...
                        Ok(None) | Err(DBError::InvalidPoxSortition) => {
                            info!("Stacks tip {}/{} affirmation map {} is not on a chain tipped by sortition {}",
                ...
                && sortition_tip_affirmation_map.len() <= canonical_affirmation_map.len()
            {
                    ...
                    canonical_affirmation_map.find_divergence(&sortition_tip_affirmation_map)
                {
                    if divergence_rc + 1 >= (heaviest_am.len() as u64) {
                        ...
                        info!("Update sortition-changed reward cycle to {} from canonical affirmation map `{}` (sortition AM is `{}`)",
    ...
    fn find_valid_sortitions(
        ...
        for height in start_height..(end_height + 1) {
            ...
            info!("{} snapshots at {}", num_sns, height);
            ...
            for (sn, sn_am) in snapshots_and_ams.into_iter() {
                ...
                if compare_am.has_prefix(&sn_am) {
                    ...
                    info!("Already processed sortition {} at height {} with AM `{}` on comparative affirmation map {}", &sn.sortition_id, sn.block_height, &sn_am, &compare_am);
                    found = true;
        ...
        for rc in changed_reward_cycle..current_reward_cycle {
            ...
            for sort_id in sort_ids.iter() {
                ...
                if prior_compare_am.has_prefix(&prior_sort_am) {
                    ...
                    found_diverged = true;
                    info!("{} diverges from {}", &sort_am, &compare_am);
    ...
    fn handle_affirmation_reorg(&mut self) -> Result<(), Error> {
            ...
            self.check_chainstate_against_burnchain_affirmations()?
        {
            ...
            if changed_reward_cycle >= sortition_reward_cycle {
                // nothing we can do
                info!("Changed reward cycle is {} but canonical sortition is in {}, so no affirmation reorg is possible", &changed_reward_cycle, sortition_reward_cycle);
                return Ok(());
            }
                ...
                match self.find_invalid_and_revalidated_sortitions(
                    ...
                    None => {
                        ...
                        if let Some(x) = revalidation_params {
                            ...
                            info!("Canonical Stacks tip for highest valid sortition {} ({}) is {}/{} height {} am `{}`", &sortition_tip, sortition_height, &canonical_ch, &canonical_bhh, canonical_height, &stacks_am);
            ...
            let valid_sortitions = if let Some(ref first_sn) = valid_sortitions.first() {
                if first_sn.block_height > sortition_height {
                    info!("No sortitions to revalidate: highest is {},{}, first candidate is {},{}. Will not revalidate.", sortition_height, &sortition_tip, first_sn.block_height, &first_sn.sortition_id);
                    vec![]
                } else {
                    valid_sortitions
                }
            ...
            info!("Invalidate all descendants of {} (after height {}), revalidate some sortitions at and after height {}, and retry all orphaned Stacks blocks at or after height {}",
            ...
            self.sortition_db.invalidate_descendants_with_closures(
                ...
                |sort_tx| {
                    ...
                    info!("Canonical Stacks tip after invalidations is {}/{} height {} am `{}`", &canonical_ch, &canonical_bhh, canonical_height, &stacks_am);
                    ...
                    info!("Canonical Stacks tip after invalidations and revalidations is {}/{} height {} am `{}`", &canonical_ch, &canonical_bhh, canonical_height, &stacks_am);
                    ...
                    info!("Canonical Stacks tip after invalidations, revalidations, and processed dirty snapshots is {}/{} height {} am `{}`", &canonical_ch, &canonical_bhh, canonical_height, &stacks_am);
    ...
    pub fn handle_new_epoch2_burnchain_block(
        ...
        info!("Handle new burnchain block");
        ...
        for unprocessed_block in sortitions_to_process.into_iter() {
            ...
            let mut compatible_stacks_blocks = vec![];
            {
                ...
                for (ch, bhh, height) in stacks_blocks_to_reaccept.into_iter() {
                    ...
                    if StacksChainState::is_block_compatible_with_affirmation_map(
                        ...
                        info!("Stacks block {}/{} height {} is NOT compatible with `{}`; will NOT reaccept", &ch, &bhh, height, &heaviest_am);
                    }
                }
            }
            ...
            for (ch, bhh, height) in compatible_stacks_blocks.into_iter() {
                info!("Re-accept Stacks block {}/{} height {}", &ch, &bhh, height);
            ...
            if revalidated_stacks_block {
                info!("Bump Stacks block(s) reprocessed");
        ...
        info!("Done handling new burnchain blocks");
    ...
    fn replay_stacks_blocks(
        ...
        for bhh in blocks.into_iter() {
            ...
            info!("Consider replaying {} from {:?}", &bhh, &staging_block_chs);
            ...
            for alt_ch in staging_block_chs.into_iter() {
                    ...
                    StacksChainState::load_block(&self.chain_state_db.blocks_path, &alt_ch, &bhh)
                {
                        ...
                        .unwrap_or(None)
                    {
                        ...
                        info!("Replay Stacks block from {} to {}/{}", &alt_ch, &ch, &bhh);
                        let ic = self.sortition_db.index_conn();
    ...
    fn check_pox_anchor_affirmation(
        ...
        if BurnchainDB::is_anchor_block(
                ...
                == &AffirmationMapEntry::PoxAnchorBlockPresent
            {
                ...
                info!("Discovered an old anchor block: {} (height {}, rc {}) with heaviest affirmation map {}", pox_anchor, commit.block_height, reward_cycle, &heaviest_am);
                info!("Discovered an old anchor block: {}", pox_anchor);
                return Ok(Some(pox_anchor.clone()));
            } else {
                // nope -- can ignore
                info!("Discovered unaffirmed old anchor block: {} (height {}, rc {}) with heaviest affirmation map {}", pox_anchor, commit.block_height, reward_cycle, &heaviest_am);
                return Ok(None);
            }
        } else {
            info!("Stacks block {} received F*w confirmations but is not the heaviest-confirmed burnchain block, so treating as non-anchor block", pox_anchor);
    ...
    fn process_ready_blocks(&mut self) -> Result<Option<BlockHeaderHash>, Error> {
        ...
        while let Some(block_result) = processed_blocks.pop() {
            if block_result.0.is_none() && block_result.1.is_none() {
                ...
                info!("Bump blocks processed (invalid)");
                self.notifier.notify_stacks_block_processed();
                ...
                if in_sortition_set {
                    ...
                    info!("Bump blocks processed ({})", &new_canonical_stacks_block);
