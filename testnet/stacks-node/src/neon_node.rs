    ...
    pub fn run_tenure(&mut self) -> Option<MinerThreadResult> {
        ...
        if res.is_none() {
            ...
            if !self.config.node.mock_mining {
                warn!("Relayer: will attempt to resubmit Bitnet IO transaction on the next passing block confirmation");
                return None;
            }
            debug!("Relayer: Mock-mining enabled; not sending Bitnet IO transaction");
        } else {
